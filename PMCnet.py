...
# I claim we dont need est_ml because it is only needed to determine the number of parameters
...
def SL_PMC_Adapt_Cov_new(train_loader,N,K,T,sig_prop,lr,gr_period,tp,est_ml,epsilon1, epsilon2):
    ...
    for t in range(T):
        ...
        for s in range(sample_number):
            ...
            print(f'Completed evaluation of sample {s} of {sample_number} for iteration {t}')
        ...
        for n in range(N):
            ...
            for x_train, y_train in train_loader:
                ...
                if flag_A == 0:
                    ...
                    Save_A_temp = A_n
                print("Keys in W_g:", W_g.keys())
                print("Keys in B_g:", B_g.keys())
                dirn = Param2vec(W_g,B_g)
                ...
                batch += 1
            if max_trials_reached_count > 0:
                print(f'We failed to find a step size for proposal {n} in iteration {t} for {max_trials_reached_count} out of 72')  

        print(f'Completed Langevin adaptation for iteration {t}')
        print(['Iteration ',t,': we discard ',ct,'/',N*(batch+1),' Sigma matrices'])

    return Mean_est_evolution,Z_est_evolution,all_samples, all_weights,all_proposals,all_Sigma,mean_est_part,Z_part,m2_est_evolution,m2_part,samples_resampled_adapted,Sigma_resampled,logf_save
